...
quantities = mp.Pool(NUM_PROCS).map(get_quantitiy_of_interest, samples)
...
mc_params = pd.concat(params, keys=range(NUM_DRAWS), names=['iteration'])

mc_quantities = pd.DataFrame(quantities, columns=['avg_schooling'], index=range(NUM_DRAWS))
